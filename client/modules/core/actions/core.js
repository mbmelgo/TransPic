export default {
  ...
  addTranslation({Meteor, LocalState, FlowRouter, Collections}, formData){
    ...
    if(!(formData.image instanceof window.File) && !(formData.image instanceof window.Blob)) return LocalState.set('ADD_TRANSLATION_ERROR','Atleast 1 image of the word is required');
  ...
  clearAddTranslationErrors({LocalState}){
    return LocalState.set({"ADD_TRANSLATION_ERROR": null});
  },
